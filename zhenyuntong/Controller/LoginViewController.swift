...
class LoginViewController: UIViewController , UITextFieldDelegate {
    ...
    @IBAction func getCode(_ sender: Any) {
        ...
        NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.appCheckPhone, params: ["phone" : mobile!]){
            [weak self] (json , error) in
            if let object = json {
                if let result = object["status"].int , result == 1 {
                    self?.getCode(hud: hud, mobile: mobile!)
    ...
    func getCode(hud : MBProgressHUD, mobile : String) {
        NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.appSendNews, params: ["phone" : mobile]){
            (json , error) in
            hud.hide(animated: true)
            if let object = json {
                if let message = object["info"].string , message.characters.count > 0 {
    ...
    @IBAction func doLogin(_ sender: Any) {
        ...
        NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.login, params: ["phone": mobile!, "verify": code!]){
            [weak self] (json , error) in
            hud.hide(animated: true)
            if let object = json {
                if let result = object["status"].int , result == 1 {
                    if var dict = object["rows"].dictionaryObject {
