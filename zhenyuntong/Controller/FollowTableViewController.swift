...
class FollowTableViewController: UITableViewController ,DZNEmptyDataSetDelegate , DZNEmptyDataSetSource {
    ...
    func loadData() {
        var params : [String : Any] = ["rows" : 20 , "page" : page, "mobile" : mobile]
        if strId.characters.count > 0 {
            params["id"] = strId
        }
        NetworkManager.installshared.request(type: .post, url:  NetworkManager.installshared.appCustfollowDetail, params: params ){
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let label = cell.viewWithTag(10) as? UILabel {
            label.text = data[indexPath.row]["callresult_name"].string
        }
        if let label = cell.viewWithTag(11) as? UILabel {
            label.text = data[indexPath.row]["fo_content"].string
        }
        if let label = cell.viewWithTag(12) as? UILabel {
            label.text = data[indexPath.row]["fo_addtime"].string
        }
        if let label = cell.viewWithTag(13) as? UILabel {
            label.text = data[indexPath.row]["handle_name"].string
        }
