...
class CommunityEventViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let attaches = item?["params" , "attach"].array
        let attachCount = attaches?.count ?? 0
        let params = item?["params" , "params"].arrayValue
    ...
    func doSubmit(sender : AnyObject)  {
        ...
        if tag == 10000 {
            ...
            alert.addAction(UIAlertAction(title: "是", style: .default, handler: {[weak self] (action) in
                ...
                NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.null, params: ["workName" : self!.title! , "sendHeader" : "/bbServer/upload/1482677327416.jpg" , "senderId" : UserDefaults.standard.integer(forKey: "userId") , "workId" : self!.item!["workId"].intValue , "sendName" : self!.item!["nickName"].stringValue , "toUser" : 2 , "areaUser" : 0]){
            ...
            alert.addAction(UIAlertAction(title: "确认", style: .default, handler: {[weak self] (action) in
                ...
                Alamofire.upload(multipartFormData: {[weak self] (data) in
                    let attaches = self?.item?["params" , "attach"].array
                    ...
                    data.append("\(self!.item!["workTypeId"].intValue)".data(using: .utf8)!, withName: "workTypeId")
                    if let json = UserDefaults.standard.object(forKey: "mine") {
                        let object = JSON(json)
                        data.append(object["nickName"].stringValue.data(using: .utf8)!, withName: "sendName")
                    }
                    ...
                    if var str = self?.item?["params"].rawString() {
                ...
                }, to: NetworkManager.installshared.macAddress() + "/bbServer/" + (self!.state == 2 ? NetworkManager.installshared.null : NetworkManager.installshared.null)) { (result) in
                    hud?.hide(animated: true)
                    switch result {
