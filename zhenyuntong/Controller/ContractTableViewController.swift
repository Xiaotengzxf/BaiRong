...
class ContractTableViewController: UITableViewController ,DZNEmptyDataSetDelegate , DZNEmptyDataSetSource {
    ...
    override func viewDidLoad() {
        ...
        loadData()
        NotificationCenter.default.addObserver(self, selector: #selector(ProductManageTableViewController.handleNotification(notification:)), name: Notification.Name("contract"), object: nil)
    }
    ...
    func loadData() {
        NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.appContract, params: ["search" : search , "page" : page]){
    ...
    @IBAction func search(_ sender: Any) {
        if let controller = self.storyboard?.instantiateViewController(withIdentifier: "search") as? SearchViewController {
            ...
            controller.searchName = "contract"
            ...
            if tag == 1 {
                if let userInfo = notification.userInfo as? [String : Any] {
                    if let message = userInfo["message"] as? String {
                        if let controller = self.storyboard?.instantiateViewController(withIdentifier: "contract") as? ContractTableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let label = cell.viewWithTag(3) as? UILabel {
            label.text = "模板：\(data[indexPath.row]["cmid"].stringValue)"
        }
        ...
        if let label = cell.viewWithTag(5) as? UILabel {
            label.text = "\(data[indexPath.row]["remarks"].stringValue)"
        }
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        row = indexPath.row
        self.performSegue(withIdentifier: "contractdetail", sender: self)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let controller = segue.destination as? ContractDetailTableViewController {
