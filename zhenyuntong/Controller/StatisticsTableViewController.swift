...
class StatisticsTableViewController: UITableViewController ,DZNEmptyDataSetDelegate , DZNEmptyDataSetSource, StatisticsSearchViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(StatisticsTableViewController.handleNotification(notification:)), name: Notification.Name("StatisticsTableViewController"), object: nil)
    ...
    func loadData() {
        ...
        NetworkManager.installshared.request(type: .post, url:  NetworkManager.installshared.appReportFigure, params: params ){
    ...
    @IBAction func search(_ sender: Any) {
        if let controller = self.storyboard?.instantiateViewController(withIdentifier: "StatisticsSearch") as? StatisticsSearchViewController {
            controller.modalTransitionStyle = .crossDissolve
            controller.view.backgroundColor = UIColor.black.withAlphaComponent(0.5)
            controller.modalPresentationStyle = .overFullScreen
            controller.delegate = self
            controller.stime = stime
            controller.etime = etime
            controller.exten = exten
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let label = cell.viewWithTag(10) as? UILabel {
            label.text = data[indexPath.row]["extenname"].string
        }
        if let label = cell.viewWithTag(11) as? UILabel {
            label.text = data[indexPath.row]["extension"].string
        }
        if let label = cell.viewWithTag(12) as? UILabel {
            label.text = data[indexPath.row]["successnum"].string
        }
        if let label = cell.viewWithTag(13) as? UILabel {
            label.text = data[indexPath.row]["tatoltime"].string
        }

        if let label = cell.viewWithTag(15) as? UILabel {
            label.text = data[indexPath.row]["tatolnum"].string
        }
        if let label = cell.viewWithTag(16) as? UILabel {
            label.text = data[indexPath.row]["failnum"].string
        }
        if let label = cell.viewWithTag(17) as? UILabel {
            label.text = data[indexPath.row]["num"].string
        }
