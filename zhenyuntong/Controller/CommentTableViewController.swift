...
class CommentTableViewController: UITableViewController , WKUIDelegate , WKNavigationDelegate{
    ...
    func loadData() {
        ...
        NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.null, params: ["newsId" : adId , "pageIndex" : page , "userId" : userId]){
    ...
    func loadNewsData() {
        let userId = UserDefaults.standard.integer(forKey: "userId")
        NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.null, params: ["newsId" : adId , "userId" : userId]){
            [weak self] (json , error) in
            if let object = json {
                if let result = object["result"].int , result == 1000 {
    ...
    @IBAction func doEvent(_ sender: Any) {
        ...
        if let bar = sender as? UIBarButtonItem {
            ...
            case 3:
                ...
                NetworkManager.installshared.request(type: .post, url: isFavorities ? NetworkManager.installshared.null : NetworkManager.installshared.null, params: ["userId" : userId , "newsId" : adId]){
                    [weak self] (json , error) in
                    self?.singleTap = true
        ...
        if let tag = sender.object as? Int {
            if tag == 1 {
                if let dictionary = sender.userInfo as? [String : String] {
                    let userId = UserDefaults.standard.integer(forKey: "userId")
                    let hud = showHUD(text: "加载中...")
                    NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.null , params: ["userId" : userId , "newsId" : adId , "content" : dictionary["content"] ?? ""]){
                        [weak self] (json , error) in
                        hud.hide(animated: true)
                        if let object = json {
                            if let result = object["result"].int , result == 1000 {
