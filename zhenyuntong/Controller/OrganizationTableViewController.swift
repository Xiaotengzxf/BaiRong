...
class OrganizationTableViewController: UITableViewController, DZNEmptyDataSetSource , DZNEmptyDataSetDelegate {
    ...
    override func viewDidLoad() {
        ...
        loadData()
        NotificationCenter.default.addObserver(self, selector: #selector(OrganizationTableViewController.handleNotification(notification:)), name: Notification.Name("org"), object: nil)
    }
    ...
    func loadData() {
        NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.appDepartment, params: nil){
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if let b = data[section]["open"].bool , b == true {
            return data[section]["users"].array?.count ?? 0
        }
        return 0
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let imageView = cell.viewWithTag(2) as? UIImageView {
            imageView.sd_setImage(with: URL(string: data[indexPath.section]["users" , indexPath.row ,  "img"].stringValue), placeholderImage: nil)
        }
        if let label = cell.viewWithTag(3) as? UILabel {
            label.text = data[indexPath.section]["users" , indexPath.row , "name"].string
        }
        if let label = cell.viewWithTag(4) as? UILabel {
            label.text = "分机号：" + data[indexPath.section]["users" , indexPath.row ,  "extension"].stringValue
        }
