...
class CommonTableViewController: UITableViewController , SDCycleScrollViewDelegate {
    ...
    func loadData()  {
        if item == nil {
            ...
            NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.null, params: ["userId" : userId , "pageIndex" : page]){
            ...
            NetworkManager.installshared.request(type: .post, url: NetworkManager.installshared.null, params: ["itemId" : itemId , "isArea" : isArea , "areaId" : areaId , "pageIndex" : page]){
                [weak self] (json , error) in
                self?.tableView.mj_header.endRefreshing()
                self?.tableView.mj_footer.endRefreshing()
                if var object = json {
